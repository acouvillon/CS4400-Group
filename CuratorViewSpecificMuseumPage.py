...
class BMTRSApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames ={}
        for F in {CuratorViewSpecificMuseumPage}:
        ...
        self.show_frame(CuratorViewSpecificMuseumPage)
...
class CuratorViewSpecificMuseumPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
            ...
            review_museum_button = tk.Button(self, text="Review Museum", fg='blue',
                                        command=lambda: controller.show_frame(MuseumReviewPage))
            #if review has already been created- that needs to come up here ^^
            ...
            #needs to be the reviews for the museum, not the visitor's reviews^^
            add_exhibit_button = tk.Button(self, text="Add Exhibit", fg='blue',
                                        command=lambda: controller.show_frame(NewExhibitPage))
            back_button = tk.Button(self, text="Back", fg='blue', borderwidth=0,
                                        command=lambda: controller.show_frame(MyMuseumsPage))

            purchase_ticket_button.pack(anchor='n', expand=True)
            review_museum_button.pack(pady=5, anchor='n', expand=True)
            view_other_reviews_button.pack(pady=5, anchor='n', expand=True)
